...
- ✅ 2026-07-06 | Windows 窗口四角能看到一圈直角（背景不透明后明显） | `apply_acrylic` 给整个**矩形**窗口加亚克力（无圆角参数），而 `.app` 有 `border-radius:12px` 圆角，圆角内容裁进去后四角露出下层直角亚克力 | 前端 `navigator.userAgent` 含 Windows 时给根加 `os-windows` 类，`.os-windows .app{border-radius:0}` 让内容填满矩形（盖住整层亚克力）；窗口圆角交给系统 DWM：`set_window_rounded` 调 `DwmSetWindowAttribute(DWMWA_WINDOW_CORNER_PREFERENCE=33, DWMWCP_ROUND=2)`（Win11 圆角 / Win10 直角，无害）。mac 不变（vibrancy 自带圆角）
- ✅ 2026-07-06 | Windows 上呼出几次后窗口「一闪而过」 | `auto_hide_on_blur` 默认 true；Windows show 瞬间窗口常先来一发 `Focused(false)`（还没稳定拿到前台焦点，反复请求前台被系统节流后更明显）→ 立即隐藏 | 加「刚显示保护期」：`Inner.last_shown_at` 在 `show_main_window` 打时间戳，`Focused(false)` 隐藏前判断距上次显示 <500ms 则忽略隐藏
- ✅ 2026-07-06 | 设置窗口不能滚动、下面的设置项看不到 | `.tab-panel.active` 有 `overflow:hidden`，作为 `.settings-body`（纵向 flex）的 item 时，`overflow` 非 visible 使其 `min-height:auto` 解析为 0 → 内容超高被压缩并裁掉下面行，父容器反而无溢出、不出滚动条 | 给 `.tab-panel.active` 加 `flex:0 0 auto`（固定为内容高度、不收缩），撑开 `.settings-body` 出滚动条。跨平台通用规范坑
- ✅ 2026-07-06 | Windows 窗口总是半透明、要求默认不透明+可调 | 窗口 `transparent:true`+亚克力，`.app` 背景 transparent，永远透出磨砂 | 新增设置 `windowOpacity`（40..=100，默认 100=不透明）；`.app` 背景改 `color-mix(in srgb, var(--bg) var(--app-opacity,100%), transparent)`（带实心 var(--bg) fallback 保证默认不透明），前端 `applyAppearance` 设 `--app-opacity`；外观 Tab 加「背景不透明度」滑块。调低才透出毛玻璃/亚克力。**注**：mac 默认也变不透明（毛玻璃需自行把不透明度调低）
